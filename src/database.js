...
async function _createTables() {
  const stmts = [
    ...
    `CREATE TABLE IF NOT EXISTS planos (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      tipo TEXT UNIQUE NOT NULL, -- semanal, mensal, bimestral
      preco REAL NOT NULL,
...
async function totalAuthUsers() { return (await getAsync(`SELECT COUNT(*) as total FROM auth_users`))?.total || 0; }
  ...
  // Planos
  getPlanos: async () => queryAsync(`SELECT * FROM planos`),
  getPlano: async (tipo) => getAsync(`SELECT * FROM planos WHERE tipo=?`, [tipo]),
  setPlano: async (tipo, preco, estoque) => _exec(`INSERT INTO planos (tipo, preco, estoque) VALUES (?,?,?) ON CONFLICT(tipo) DO UPDATE SET preco=excluded.preco, estoque=excluded.estoque, atualizado_em=datetime('now','localtime')`, [tipo, preco, estoque]),
  decrementarEstoque: async (tipo) => _exec(`UPDATE planos SET estoque = MAX(0, estoque - 1) WHERE tipo=?`, [tipo]),
};
