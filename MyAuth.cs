...
namespace MyAuth
{
    public class api
    {
        ...
        {
            if (_authKey == null && user_data?.auth_key == null) return;
            if (_heartbeatTimer != null) return;

            string keyToUse = _authKey ?? user_data?.auth_key;
            if (string.IsNullOrEmpty(keyToUse)) return;

            _heartbeatTimer = new System.Windows.Forms.Timer();
            _heartbeatTimer.Interval = 30000; // MUDEI para 30 segundos (antes era 60)
            _heartbeatTimer.Tick += (sender, e) =>
            {
                try
                {
                    var json = Post("/auth/heartbeat", new { auth_key = keyToUse });
                    var obj = JObject.Parse(json);

                    bool ok = obj["success"]?.Value<bool>() ?? false;

                    // ADICIONEI LOG para debug
                    System.Diagnostics.Debug.WriteLine($"[Heartbeat] Success: {ok} - Response: {json}");
                    ...
                    if (!ok)
                    {
                        ...
                        System.Windows.Forms.MessageBox.Show(
                            msg + "\n\nSessão expirou. O Auth ID pode não ser permanente no servidor.",
                            "Sessão Encerrada",
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"[Heartbeat] ERRO: {ex.Message}");
                    // Não faz nada, tenta de novo
                }
            };
            _heartbeatTimer.Start();

            System.Diagnostics.Debug.WriteLine("[Heartbeat] Iniciado com intervalo de 30 segundos");
        }
